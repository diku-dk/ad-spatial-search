...
def diterationSorted_ALL_inlined [q][n][d][num_leaves][ppl][r]
  ...
  let (x_ws_sorted_bar, y_ws_sorted_bar) =
    map2 (\(x, x_w, y, y_w) leaf_ind ->
      -- Primal unneeded.
      -- Rev.
      let res0bars: [r][r]f32 = resbars
      ...
      let x_w_bar =
        ...
        then (x_w_bar, y_w_bar)
          ...
          #[sequential] dbruteForce_opt_seq_ALL radiuses x x_w y y_w x_w_bar y_w_bar res0bars
      in x_w_bar
  ...
  -- TODO All speed-up over AD is lost on the below histogram computations.
  let y_ws_bars =
